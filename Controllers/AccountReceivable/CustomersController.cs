...
namespace VenusERP_API.Controllers.AccountReceivable
{
    public class CustomersController : BaseApiController
    {
        ...
        public ActionResult SearchCustomers(string? ExciptIds = "", string? Filter = "")
        {
            try
            {
        ...
        public ActionResult GetList(int? pageSize, int? pageNumber, string? orderBy = "", string? orderDirection = "", string? criteria = "")
        {
            try
            {
                ...
                var x = DataHelper.ExecScalar("fcs_CustomersGetList", System.Data.CommandType.StoredProcedure, l.ToArray());
                if (string.IsNullOrEmpty(x.ToString()))
                {
                    return StatusCode(200, "");
                }
        ...
        public ActionResult GetByID(int ID)
        {
            try
            {
                ...
                DataHelper.ExecScalar("fcs_GetCustomerDetails", System.Data.CommandType.StoredProcedure, ref listOutPutParams, l.ToArray());
        ...
        public ActionResult GetNextCode()
        {
            try
            {
                ...
                var result = DataHelper.ExecScalar(@"select CAST( isnull((max(cast(code as bigint))+1),1) AS VARCHAR(20))
                                                       from fcs_Customers WHERE ISNUMERIC(code) = 1"
        ...
        public ActionResult CheckCodeExist(string? code)
        {
            try
            {
                ...
                var result = DataHelper.ExecScalar(@"select count(*) from fcs_Customers WHERE code = @Code"
        ...
        public ActionResult Save(DataObj dataObj)
        {
            try
            {
                ...
                DataHelper.ExecScalar("fcs_CustomersSave", System.Data.CommandType.StoredProcedure, ref listOutParameters, l.ToArray());
        ...
        public ActionResult Delete(DataObj dataObj)
        {
            try
            {
                ...
                DataHelper.ExecScalar("fcs_CustomersDelete", System.Data.CommandType.StoredProcedure, ref listOutParameters, l.ToArray());
                string error = listOutParameters.Find(X => X.ParameterName == "@Error").Value.ToString();
                if (!string.IsNullOrEmpty(error))
                    return StatusCode(500, error);
        ...
        public ActionResult CustomerInfo(int CustomerId)
        {
            try
            {
                ...
                var x = DataHelper.ExecScalar("fcs_CustomerInfo", System.Data.CommandType.StoredProcedure, l.ToArray());
                if (string.IsNullOrEmpty(x.ToString()))
                {
