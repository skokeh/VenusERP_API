...
{
    public class Asset_MaintenanceController : BaseApiController
    {
        [HttpGet("GetList")]
        public ActionResult GetList(int pageSize, int pageNumber, string? orderBy = "", string? orderDirection = "", string? criteria = "")
        {
            try
            {
                ...
                var x = DataHelper.ExecScalar("Asset_Maintenance_GetList", System.Data.CommandType.StoredProcedure, l.ToArray());
        ...
        public ActionResult GetByID(int ID)
        {
            try
            {
                ...
                DataHelper.ExecScalar("Asset_Maintanence_GetByID", System.Data.CommandType.StoredProcedure, ref listOutPutParams, l.ToArray());
        ...
        public ActionResult GetNextCode()
        {
            try
            {
                ...
                var result = DataHelper.ExecScalar(@"select CAST( isnull((max(cast(code as bigint))+1),1) AS VARCHAR(20))
                                                       from Asset_MaintenanceHead WHERE ISNUMERIC(code) = 1"
        ...
        public ActionResult CheckCodeExist(string code)
        {
            try
            {
                ...
                var result = DataHelper.ExecScalar(@"select count(*) from Asset_MaintenanceHead WHERE code = @Code"
        ...
        public ActionResult Save(DataObj dataObj)
        {
            try
            {
                ...
                string pName = "";
                foreach (var item in oo.Properties().Where(x => x.Name.ToLower() != "remainderData").ToArray())
                {
                    pName = ((Newtonsoft.Json.Linq.JProperty)item).Name.ToString().ToLower();
                    headJson.Add(item);
                }
                var remainderData = oo["remainderData"];
                ...
                DataHelper.ExecScalar("Asset_MaintanenceSave", System.Data.CommandType.StoredProcedure, ref listOutParameters, l.ToArray());
                ...
                if( remainderData.HasValues)
                {
                    JObject remJson = new JObject();
                    remJson.Add("Head", remainderData);
                    List<SqlParameter> la = new List<SqlParameter>();
                    la.Add(new SqlParameter("@User", GetCurrentUser()));
                    la.Add(new SqlParameter("@Lang", ExtractLang()));
                    la.Add(new SqlParameter("@json", remJson.ToString()) { Size = -1 });
                    ...
                    DataHelper.ExecScalar("sys_RemindersSave", System.Data.CommandType.StoredProcedure, ref listOutParameters1, la.ToArray());
                    string error1 = listOutParameters1.Find(X => X.ParameterName == "@Error").Value.ToString();
                    if (!string.IsNullOrEmpty(error1))
                        return StatusCode(500, error1);
                    string result1 = listOutParameters1.Find(X => X.ParameterName == "@Result").Value.ToString();

                    if (string.IsNullOrEmpty(result1))
                    {
                        return StatusCode(500, result1);
                    }
                }
        ...
        public ActionResult Delete(DataObj dataObj)
        {
            try
            {
                ...
                DataHelper.ExecScalar("Ass_Items_Delete", System.Data.CommandType.StoredProcedure, ref listOutParameters, l.ToArray());
                string error = listOutParameters.Find(X => X.ParameterName == "@Error").Value.ToString();
                if (!string.IsNullOrEmpty(error))
                    return StatusCode(500, error);
