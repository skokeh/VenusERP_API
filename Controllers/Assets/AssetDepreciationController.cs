...
{
    public class AssetDepreciationController : BaseApiController
    {
        [HttpGet("GetList")]
        public ActionResult GetList(int pageSize, int pageNumber, string? orderBy = "", string? orderDirection = "", string? criteria = "")
        {
            try
            {
                ...
                var x = DataHelper.ExecScalar("Ass_DepreciationTrxList", System.Data.CommandType.StoredProcedure, l.ToArray());
        ...
        public ActionResult GetByID(int ID)
        {
            try
            {
                ...
                DataHelper.ExecScalar("Ass_DepreciationTrx_GetById", System.Data.CommandType.StoredProcedure, ref listOutPutParams, l.ToArray());
        ...
        [HttpGet("GetNextCode")]
        public ActionResult GetNextCode(string TransDate = "")
        {
            try
            {
                DateTime transDate = DateTime.Now;
                DateTime.TryParse(TransDate, out transDate);
                var result = DataHelper.ExecScalar(@"select ISNULL(max(Number),0)+1 from Ass_Depreciation_TrxH  where YEAR(TRANSDate) =" + transDate.Year
        ...
        public ActionResult Save(DataObj dataObj)
        {
            try
            {
                ...
                && x.Name.ToLower() != "assetDepreciationDetails".ToLower()).ToArray())
                {
                    pName = ((Newtonsoft.Json.Linq.JProperty)item).Name.ToString().ToLower();
                    headJson.Add(item);
                }
                var detailsJson = oo["assetDepreciationDetails"];
                var attachments = oo["attachments"].ToArray();

                JObject newJson = new JObject();
                newJson.Add("Head", headJson);
                newJson.Add("assetDepreciationDetails", detailsJson);
                ...
                DataHelper.ExecScalar("Ass_DepreciationTrx_Save", System.Data.CommandType.StoredProcedure, ref listOutParameters, l.ToArray());
        ...
        [HttpGet("Delete")]
        public ActionResult Delete(int AssetDepreciationTrxHId)
        {
            try
            {
                ...
                l.Add(new SqlParameter("@FormId", formId));
                l.Add(new SqlParameter("@Lang", ExtractLang()));
                l.Add(new SqlParameter("@Id", AssetDepreciationTrxHId));

                List<SqlParameter> listOutParameters = new List<SqlParameter>();
                SqlParameter errPar = new SqlParameter() { ParameterName = "@Error", SqlDbType = SqlDbType.NVarChar, Size = -1, Direction = ParameterDirection.Output };
                listOutParameters.Add(errPar);

                DataHelper.ExecScalar("Ass_DepreciationTrx_Delete", System.Data.CommandType.StoredProcedure, ref listOutParameters, l.ToArray());
                string error = listOutParameters.Find(X => X.ParameterName == "@Error").Value.ToString();
                if (!string.IsNullOrEmpty(error))
                    return StatusCode(500, error);
        ...
        [HttpGet("GetAssetDepreciationItems")]
        public ActionResult GetAssetDepreciationItems(string TransDate)
        {
            try
            {
                List<SqlParameter> l = new List<SqlParameter>();
                ...
                l.Add(new SqlParameter("@FormId", ExtractFormId()));
                l.Add(new SqlParameter("@Lang", ExtractLang()));
                l.Add(new SqlParameter("@TransDate", TransDate));

                var x = DataHelper.ExecScalar("GetAssetDepreciationItems", System.Data.CommandType.StoredProcedure, l.ToArray());
                if (string.IsNullOrEmpty(x.ToString()))
                {
                    return StatusCode(200, "");
                }
