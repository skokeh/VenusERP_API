...
{
    public class VendorNoteController : BaseApiController
    {
        [HttpGet("GetList")]
        public ActionResult GetList(int pageSize, int pageNumber, string? orderBy = "", string? orderDirection = "", string? criteria = "")
        {
            try
            {
                ...
                var x = DataHelper.ExecScalar("fcs_VendorNoteList", System.Data.CommandType.StoredProcedure, l.ToArray());
        ...
        public ActionResult GetByID(int ID)
        {
            try
            {
                ...
                DataHelper.ExecScalar("fcs_VendorNoteDetails", System.Data.CommandType.StoredProcedure, ref listOutPutParams, l.ToArray());
        ...
        [HttpGet("GetNextCode")]
        public ActionResult GetNextCode(int TransTypeID,string NoteDate)
        {
            try
            {
                      List<SqlParameter> l = new List<SqlParameter>();
                ...
                l.Add(new SqlParameter("@TransDate", NoteDate));
                l.Add(new SqlParameter("@TableName", "fcs_VendorNote"));
                List<SqlParameter> listOutPutParams = new List<SqlParameter>();
        ...
        public ActionResult Save(DataObj dataObj)
        {
            try
            {
                ...
                string pName = "";
                foreach (var item in oo.Properties().Where(x => x.Name.ToLower() != "attachments" && x.Name.ToLower() != "details").ToArray())
                {
                    pName = ((Newtonsoft.Json.Linq.JProperty)item).Name.ToString().ToLower();
                    headJson.Add(item);
                }
                var detailsJson = oo["details"];
                var attachments = oo["attachments"].ToArray();

                JObject newJson = new JObject();
                newJson.Add("Head", headJson);
                newJson.Add("details", detailsJson);
                ...
                DataHelper.ExecScalar("fcs_VendorNoteSave", System.Data.CommandType.StoredProcedure, ref listOutParameters, l.ToArray());
        ...
        public ActionResult Delete(DataObj dataObj)
        {
            try
            {
                ...
                DataHelper.ExecScalar("fcs_VendorNoteDelete", System.Data.CommandType.StoredProcedure, ref listOutParameters, l.ToArray());
                string error = listOutParameters.Find(X => X.ParameterName == "@Error").Value.ToString();
                if (!string.IsNullOrEmpty(error))
                    return StatusCode(500, error);
