...
namespace VenusERP_API.Controllers.AP
{
    public class VendorsController : BaseApiController
    {
        [HttpGet("SearchVendors")]
        public ActionResult SearchVendors(string? ExciptIds = "", string? Filter = "")
        {
            try
            {
        ...
        public ActionResult GetAllVendors()
        {
            try
            {
                ...
                var x = DataHelper.ExecScalar("fcs_getAllVendors", System.Data.CommandType.StoredProcedure, l.ToArray());
                if (string.IsNullOrEmpty(x.ToString()))
                {
                    return StatusCode(200, "");
                }
        ...
        public ActionResult GetList(int pageSize, int pageNumber, string? orderBy = "", string? orderDirection = "", string? criteria = "")
        {
            try
            {
                ...
                var x = DataHelper.ExecScalar("fcs_VendorsGetList", System.Data.CommandType.StoredProcedure, l.ToArray());
                if (string.IsNullOrEmpty(x.ToString()))
                {
                    return StatusCode(200, "");
                }
        ...
        public ActionResult GetByID(int ID)
        {
            try
            {
                ...
                DataHelper.ExecScalar("fcs_GetVendorDetails", System.Data.CommandType.StoredProcedure, ref listOutPutParams, l.ToArray());
        ...
        public ActionResult GetNextCode()
        {
            try
            {
                ...
                var result = DataHelper.ExecScalar(@"select CAST( isnull((max(cast(code as bigint))+1),1) AS VARCHAR(20))
                                                       from fcs_Vendors WHERE ISNUMERIC(code) = 1"
        ...
        public ActionResult CheckCodeExist(string code)
        {
            try
            {
                ...
                var result = DataHelper.ExecScalar(@"select count(*) from fcs_Vendors WHERE code = @Code"
        ...
        public ActionResult Save(DataObj dataObj)
        {
            try
            {
                ...
                string pName = "";
                foreach (var item in oo.Properties().Where(x => x.Name.ToLower() != "attachments" && x.Name.ToLower() != "imageData").ToArray())
                {
                ...
                DataHelper.ExecScalar("fcs_VendorsSave", System.Data.CommandType.StoredProcedure, ref listOutParameters, l.ToArray());
        ...
        public ActionResult Delete(DataObj dataObj)
        {
            try
            {
                ...
                DataHelper.ExecScalar("fcs_VendorsDelete", System.Data.CommandType.StoredProcedure, ref listOutParameters, l.ToArray());
                string error = listOutParameters.Find(X => X.ParameterName == "@Error").Value.ToString();
                if (!string.IsNullOrEmpty(error))
                    return StatusCode(500, error);
        ...
        public ActionResult VendorInfo(int VendorId)
        {
            try
            {
                ...
                var x = DataHelper.ExecScalar("fcs_VendorInfo", System.Data.CommandType.StoredProcedure, l.ToArray());
                if (string.IsNullOrEmpty(x.ToString()))
                {
        ...
        [HttpGet("GetVendorContactInfo")]
        public ActionResult GetVendorContactInfo(int VendorId)
        {
            try
            {
                List<SqlParameter> l = new List<SqlParameter>();
                ...
                var x = DataHelper.ExecScalar("fcs_GetVendorContactInfo", System.Data.CommandType.StoredProcedure, l.ToArray());
                if (string.IsNullOrEmpty(x.ToString()))
                {
                    return StatusCode(500, "");
                }
