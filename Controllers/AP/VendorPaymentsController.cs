...
{
    public class VendorPaymentsController : BaseApiController
    {
        [HttpGet("GetList")]
        public ActionResult GetList(int pageSize, int pageNumber, string? orderBy = "", string? orderDirection = "", string? criteria = "")
        {
            try
            {
                ...
                var x = DataHelper.ExecScalar("fcs_VendorPaymentsGetList", System.Data.CommandType.StoredProcedure, l.ToArray());
        ...
        [HttpGet("GetVendorDues")]
        public ActionResult GetVendorDues(string? VendoromerID, int? TransTypeID, int? CurrencyID)
        {
            try
            {
                ...
                l.Add(new SqlParameter("@FormId", ExtractFormId()));
                l.Add(new SqlParameter("@Lang", ExtractLang()));
                l.Add(new SqlParameter("@VendoromerID", VendoromerID));
                l.Add(new SqlParameter("@TransTypeID", TransTypeID));
                l.Add(new SqlParameter("@CurrencyID", CurrencyID));

                var x = DataHelper.ExecScalar("fcs_VendorPaymentsGetVendorDues", System.Data.CommandType.StoredProcedure, l.ToArray());
                if (string.IsNullOrEmpty(x.ToString()))
                {
                    return StatusCode(200, "");
                }
        ...
        public ActionResult GetByID(int ID)
        {
            try
            {
                ...
                DataHelper.ExecScalar("fcs_VendorPaymentDetails", System.Data.CommandType.StoredProcedure, ref listOutPutParams, l.ToArray());
        ...
        [HttpGet("GetNextCode")]
        public ActionResult GetNextCode(int TransTypeID, string PaymentDate)
        {
            try
            {
                ...
                l.Add(new SqlParameter("@FormId", ExtractFormId()));
                l.Add(new SqlParameter("@TransTypeID", TransTypeID));
                l.Add(new SqlParameter("@TransDate", PaymentDate));
                l.Add(new SqlParameter("@TableName", "fcs_VendorPayments"));
                List <SqlParameter> listOutPutParams = new List<SqlParameter>();
        ...
        public ActionResult getTransTypes()
        {
            try
            {
                ...
                var x = DataHelper.ExecScalar("fcs_GetVendorTransTypes", System.Data.CommandType.StoredProcedure, l.ToArray());
                if (string.IsNullOrEmpty(x.ToString()))
                {
                    return StatusCode(200, "");
                }
        ...
        [HttpGet("srchVendors")]
        public ActionResult srchVendors( string? ExciptIds = "", string? Filter = "")
        {
            try
            {
                List<SqlParameter> l = new List<SqlParameter>();
                ...
                var x = DataHelper.ExecScalar("fcs_srchVendors", System.Data.CommandType.StoredProcedure, l.ToArray());
                if (string.IsNullOrEmpty(x.ToString()))
                {
                    return StatusCode(200, "");
                }
        ...
        public ActionResult Save(DataObj dataObj)
        {
            try
            {
                ...
                string pName = "";
                foreach (var item in oo.Properties().Where(x => x.Name.ToLower() != "attachments" && x.Name.ToLower() != "paymentdetails".ToLower()).ToArray())
                {
                    pName = ((Newtonsoft.Json.Linq.JProperty)item).Name.ToString().ToLower();
                    headJson.Add(item);
                }
                var detailsJson = oo["paymentDetails"];
                var paymentChequesJson = oo["paymentCheques"];
                var attachments = oo["attachments"].ToArray();

                JObject newJson = new JObject();
                newJson.Add("Head", headJson);
                newJson.Add("paymentDetails", detailsJson);
                newJson.Add("paymentCheques", paymentChequesJson);
                ...
                DataHelper.ExecScalar("fcs_VendorPaymentsSave", System.Data.CommandType.StoredProcedure, ref listOutParameters, l.ToArray());
        ...
        [HttpGet( "Delete")]
        public ActionResult Delete(int VendorPaymentId)
        {
            try
            {
                ...
                l.Add(new SqlParameter("@FormId", ExtractFormId()));
                l.Add(new SqlParameter("@Lang", ExtractLang()));
                l.Add(new SqlParameter("@ID", VendorPaymentId));

                List<SqlParameter> listOutParameters = new List<SqlParameter>();
                SqlParameter errPar = new SqlParameter() { ParameterName = "@Error", SqlDbType = SqlDbType.NVarChar, Size = -1, Direction = ParameterDirection.Output };
                listOutParameters.Add(errPar);
                var x = DataHelper.ExecScalar("fcs_VendorPaymentsDelete", System.Data.CommandType.StoredProcedure, ref listOutParameters, l.ToArray());
                string error = listOutParameters.Find(X => X.ParameterName == "@Error").Value.ToString();
                if (!string.IsNullOrEmpty(error))
                    return StatusCode(500, error);
