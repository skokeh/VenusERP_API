    ...
    public class FiscalYearsController : BaseApiController
    {
        ...
        public ActionResult GetList(int pageSize, int pageNumber, string? orderBy = "", string? orderDirection = "", string? criteria = "")
        {
            try
            {
                ...
                var x = DataHelper.ExecScalar("sys_FiscalYearsGetList", System.Data.CommandType.StoredProcedure, l.ToArray());
        ...
        public ActionResult GetByID(int ID)
        {
            try
            {
                ...
                DataHelper.ExecScalar("sys_FiscalYearDetails", System.Data.CommandType.StoredProcedure, ref listOutPutParams, l.ToArray());
        ...
        public ActionResult GetNextCode()
        {
            try
            {
                ...
                var result = DataHelper.ExecScalar(@"select CAST( isnull((max(cast(code as bigint))+1),1) AS VARCHAR(20))
                                                       from sys_FiscalYears WHERE ISNUMERIC(code) = 1 ", System.Data.CommandType.Text);
        ...
        [HttpPost("StartFiscalYear")]
        public ActionResult StartFiscalYear(DataObj dataObj)
        {
            try
            {
                try
                ...
                var year = oo["Year"].ToString();
                List<SqlParameter> l = new List<SqlParameter>();
                l.Add(new SqlParameter("@User", GetCurrentUser()));
                l.Add(new SqlParameter("@Lang", ExtractLang()));
                l.Add(new SqlParameter("@Year", year));
               ...
               var result= DataHelper.ExecScalar("Sys_OpenNewFiscalYears", System.Data.CommandType.StoredProcedure, ref listOutParameters, l.ToArray());
        ...
        public ActionResult Save(DataObj dataObj)
        {
            try
            {
                ...
                DataHelper.ExecScalar("sys_FiscalYearSave", System.Data.CommandType.StoredProcedure, ref listOutParameters, l.ToArray());
                string error = listOutParameters.Find(X => X.ParameterName == "@Error").Value.ToString();
                if (!string.IsNullOrEmpty(error))
                    return StatusCode(500, error);
        ...
        public ActionResult Delete(DataObj dataObj)
        {
            try
            {
                ...
                DataHelper.ExecScalar("sys_FiscalYearDelete", System.Data.CommandType.StoredProcedure, ref listOutParameters, l.ToArray());
                string error = listOutParameters.Find(X => X.ParameterName == "@Error").Value.ToString();
                if (!string.IsNullOrEmpty(error))
                    return StatusCode(500, error);
        ...
        [HttpPost("closefiscalyear")]
        public ActionResult closefiscalyear(DataObj dataObj)
        {
            try
            {
                try
                ...
                List<SqlParameter> l = new List<SqlParameter>();
                l.Add(new SqlParameter("@FiscalYearID", ((JToken)headJson["ID"]).ToString()));
                l.Add(new SqlParameter("@RegUserID", 43));
                l.Add(new SqlParameter("@CompanyID", 15));
                l.Add(new SqlParameter("@Lang", ExtractLang()));
//                l.Add(new SqlParameter("@json", newJson.ToString()) { Size = -1 });
                ...
                DataHelper.ExecScalar("fcs_MakePLVoucher", System.Data.CommandType.StoredProcedure, ref listOutParameters, l.ToArray());
                string error = listOutParameters.Find(X => X.ParameterName == "@Error").Value.ToString();
                if (!string.IsNullOrEmpty(error))
                    return StatusCode(500, error);
