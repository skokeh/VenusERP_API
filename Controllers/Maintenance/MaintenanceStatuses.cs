...
namespace VenusERP_API.Controllers.INV
{
    ...
    public class MaintenanceStatusesController : BaseApiController
    {
        [HttpGet("GetList")]
        public ActionResult GetList(int pageSize, int pageNumber, string? orderBy = "", string? orderDirection = "", string? criteria = "")
        {
            try
            {
                ...
                var x = DataHelper.ExecScalar("MNT_MaintenanceStatusesList", System.Data.CommandType.StoredProcedure, l.ToArray());
        ...
        [HttpGet("GetMaintanenceStatuses")]
        public ActionResult GetMaintanenceStatuses()
        {
            try
            {
                ...
                var x = DataHelper.ExecScalar("Mnt_GetMaintenanceStatuses", System.Data.CommandType.StoredProcedure, l.ToArray());
                if (string.IsNullOrEmpty(x.ToString()))
                {
                    return StatusCode(200, "");
                }
        ...
        public ActionResult GetByID(int ID)
        {
            try
            {
                ...
                DataHelper.ExecScalar("MNT_MaintenanceStatusDetails", CommandType.StoredProcedure, ref listOutPutParams, l.ToArray());
        ...
        public ActionResult GetNextCode()
        {
            try
            {
                ...
                var result = DataHelper.ExecScalar(@"select CAST( isnull((max(cast(code as bigint))+1),1) AS VARCHAR(20))
                                                       from mnt_MaintenanceStatus WHERE ISNUMERIC(code) = 1"
                                    , CommandType.Text);
        ...
        public ActionResult CheckCodeExist(string code)
        {
            try
            {
                ...
                var result = DataHelper.ExecScalar(@"select count(*) from mnt_MaintenanceStatus WHERE code = @Code", CommandType.Text, l.ToArray());
        ...
        public ActionResult Save(DataObj dataObj)
        {
            try
            {
                ...
                DataHelper.ExecScalar("mnt_MaintenanceStatusesSave", System.Data.CommandType.StoredProcedure, ref listOutParameters, l.ToArray());
        ...
        public ActionResult Delete(DataObj dataObj)
        {
            try
            {
                ...
                DataHelper.ExecScalar("MNT_MaintenanceStatusesDelete", CommandType.StoredProcedure, ref listOutParameters, l.ToArray());
                string error = listOutParameters.Find(X => X.ParameterName == "@Error").Value.ToString();
                if (!string.IsNullOrEmpty(error))
                    return StatusCode(500, error);
