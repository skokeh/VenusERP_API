...
{
    public class OperationsTypesController : BaseApiController
    {
        [HttpGet("GetList")]
        public ActionResult GetList(int? pageSize, int? pageNumber, string? orderBy = "", string? orderDirection = "", string? criteria = "")
        {
            try
            {
                ...
                var x = DataHelper.ExecScalar("mms_OperationsTypeList", System.Data.CommandType.StoredProcedure, l.ToArray());
        ...
        public ActionResult GetByID(int ID)
        {
            try
            {
                ...
                DataHelper.ExecScalar("mms_OperationsTypeGetDetails", System.Data.CommandType.StoredProcedure, ref listOutPutParams, l.ToArray());
        ...
        public ActionResult GetNextCode()
        {
            try
            {
                ...
                var result = DataHelper.ExecScalar(@"select CAST( isnull((max(cast(code as decimal))+1),1) AS VARCHAR(20))
                                                       from mms_OperationsTypes WHERE ISNUMERIC(code) = 1"
        ...
        public ActionResult Save(DataObj dataObj)
        {
            try
            {
                ...
                foreach (var item in oo.Properties().Where(x => x.Name.ToLower() != "attachments"
                 && x.Name.ToLower() != "ledgersLinks"
                 && x.Name.ToLower() != "locationsLimits"
                 && x.Name.ToLower() != "paymentTypes" 
                ...
                var ledgersLinks = oo["ledgersLinks"];
                var locationsLimits = oo["locationsLimits"];
                var paymentTypes = oo["paymentTypes"];
                var attachments = oo["attachments"].ToArray();

                JObject newJson = new JObject();
                newJson.Add("Head", headJson);
                newJson.Add("ledgersLinks", ledgersLinks);
                newJson.Add("locationsLimits", locationsLimits);
                newJson.Add("paymentTypes", paymentTypes);
                List<SqlParameter> l = new List<SqlParameter>();
                l.Add(new SqlParameter("@User", GetCurrentUser()));
                ...
                DataHelper.ExecScalar("mms_OperationsTypeSave", System.Data.CommandType.StoredProcedure, ref listOutParameters, l.ToArray());
        ...
        public ActionResult Delete(DataObj dataObj)
        {
            try
            {
                ...
                DataHelper.ExecScalar("mms_OperationsTypeDelete", System.Data.CommandType.StoredProcedure, ref listOutParameters, l.ToArray());
                string error = listOutParameters.Find(X => X.ParameterName == "@Error").Value.ToString();
                if (!string.IsNullOrEmpty(error))
                    return StatusCode(500, error);
        ...
        public ActionResult GetLookups()
        {
            try
            {
                ...
                l.Add(new SqlParameter("@RoutePath", ExtractRoutePath()));
                l.Add(new SqlParameter("@Lang", ExtractLang()));
                var x = DataHelper.ExecScalar("[mms_OperationsTypeGetLookups]", System.Data.CommandType.StoredProcedure, l.ToArray());
                if (string.IsNullOrEmpty(x.ToString()))
                {
                    return StatusCode(200, "");
                }
        ...
        #region Read
        [HttpGet("GetFormOperationType")]
        public ActionResult GetFormOperationType()
        {
            try
            {
                List<SqlParameter> parameters = new List<SqlParameter>
                    {
                        ...
                        new SqlParameter("@FormId", ExtractFormId()),
                        new SqlParameter("@Lang", ExtractLang())
                    };

                var result = DataHelper.ExecScalar("mms_OperationsTypeDetails", System.Data.CommandType.StoredProcedure, parameters.ToArray());
