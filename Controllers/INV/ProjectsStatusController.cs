...
{
    public class ProjectsStatusController : BaseApiController
    {
        [HttpGet("GetList")]
        public ActionResult GetList(int pageSize, int pageNumber, string? orderBy = "", string? orderDirection = "", string? criteria = "")
        {
            try
            {
                ...
                var x = DataHelper.ExecScalar("fcs_ProjectsStatusGetList", System.Data.CommandType.StoredProcedure, l.ToArray());
        ...
        public ActionResult GetByID(int ID)
        {
            try
            {
                ...
                DataHelper.ExecScalar("fcs_ProjectsStatusDetails", System.Data.CommandType.StoredProcedure, ref listOutPutParams, l.ToArray());
        ...
        public ActionResult GetNextCode()
        {
            try
            {
                ...
                var result = DataHelper.ExecScalar(@"select CAST( isnull((max(cast(code as bigint))+1),1) AS VARCHAR(20))
                                                       from fcs_ProjectsStatus WHERE ISNUMERIC(code) = 1"
        ...
        public ActionResult CheckCodeExist(string code)
        {
            try
            {
                ...
                var result = DataHelper.ExecScalar(@"select count(*) from fcs_ProjectsStatus WHERE code = @Code"
        ...
        public ActionResult Save(DataObj dataObj)
        {
            try
            {
                ...
                DataHelper.ExecScalar("fcs_ProjectsStatusSave", System.Data.CommandType.StoredProcedure, ref listOutParameters, l.ToArray());
        ...
        public ActionResult Delete(DataObj dataObj)
        {
            try
            {
                ...
                DataHelper.ExecScalar("fcs_ProjectsStatusDelete", System.Data.CommandType.StoredProcedure, ref listOutParameters, l.ToArray());
                string error = listOutParameters.Find(X => X.ParameterName == "@Error").Value.ToString();
                if (!string.IsNullOrEmpty(error))
                    return StatusCode(500, error);
