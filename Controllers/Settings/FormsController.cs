    ...
    public class FormsController : BaseApiController
    {
        ...
        public ActionResult GetList(int pageSize, int pageNumber, string? orderBy = "", string? orderDirection = "", string? criteria = "")
        {
            try
            {
                ...
                var x = DataHelper.ExecScalar("sys_FormsGetList", CommandType.StoredProcedure, l.ToArray());
        ...
        public ActionResult GetByID(int ID)
        {
            try
            {
                ...
                DataHelper.ExecScalar("sys_FormsControlsDetails", CommandType.StoredProcedure, ref listOutPutParams, l.ToArray());
        ...
        public ActionResult Save(DataObj dataObj)
        {
            try
            {
                ...
                DataHelper.ExecScalar("sys_FormsControlsUpdate", CommandType.StoredProcedure, ref listOutParameters, l.ToArray());
        ...
        public ActionResult GetNextCode()
        {
            try
            {
                ...
                var result = DataHelper.ExecScalar(@"select CAST( isnull((max(cast(code as bigint))+1),1) AS VARCHAR(20))
                                                       from sys_Forms WHERE ISNUMERIC(code) = 1"
                                    , CommandType.Text);
        ...
        public ActionResult CheckCodeExist(string code)
        {
            try
            {
                ...
                var result = DataHelper.ExecScalar(@"select count(*) from sys_Forms WHERE code = @Code", CommandType.Text, l.ToArray());
