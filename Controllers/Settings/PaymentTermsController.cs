...
{
    public class PaymentsTermsController : BaseApiController
    {
        [HttpGet("GetList")]
        public ActionResult GetList(int pageSize, int pageNumber, string? orderBy = "", string? orderDirection = "", string? criteria = "")
        {
            try
            {
                ...
                var x = DataHelper.ExecScalar("fcs_PaymentsTermsList", CommandType.StoredProcedure, l.ToArray());
        ...
        public ActionResult GetByID(int ID)
        {
            try
            {
                ...
                DataHelper.ExecScalar("fcs_PaymentsTermDetails", CommandType.StoredProcedure, ref listOutPutParams, l.ToArray());
        ...
        public ActionResult GetNextCode()
        {
            try
            {
                ...
                var result = DataHelper.ExecScalar(@"select CAST( isnull((max(cast(code as bigint))+1),1) AS VARCHAR(20))
                                                       from fcs_PaymentsTerms WHERE ISNUMERIC(code) = 1"
                                    , CommandType.Text);
        ...
        public ActionResult CheckCodeExist(string code)
        {
            try
            {
                ...
                var result = DataHelper.ExecScalar(@"select count(*) from fcs_PaymentsTerms WHERE code = @Code", CommandType.Text, l.ToArray());
        ...
        public ActionResult GetPaymentTerms()
        {
            try
            {
                ...
                var x = DataHelper.ExecScalar("fcs_GetPaymentTerms", System.Data.CommandType.StoredProcedure, l.ToArray());
                if (string.IsNullOrEmpty(x.ToString()))
                {
                    return StatusCode(200, "");
                }
        ...
        public ActionResult Save(DataObj dataObj)
        {
            try
            {
                ...
                DataHelper.ExecScalar("fcs_PaymentsTermsSave", CommandType.StoredProcedure, ref listOutParameters, l.ToArray());
        ...
        public ActionResult Delete(DataObj dataObj)
        {
            try
            {
                ...
                DataHelper.ExecScalar("fcs_PaymentsTermsDelete", CommandType.StoredProcedure, ref listOutParameters, l.ToArray());
                string error = listOutParameters.Find(X => X.ParameterName == "@Error").Value.ToString();
                if (!string.IsNullOrEmpty(error))
                    return StatusCode(500, error);
